...
export default function PHLedger() {
  ...
  const [basQ, setBasQ]         = useState([]);
  const [gst, setGst]           = useState([]);
  const [taxAU, setTaxAU]       = useState({co:null,pe:null});
  const [taxCA, setTaxCA]       = useState({co:null,pe:null});
  const [fin, setFin]           = useState({});
  const [msgs, setMsgs]         = useState([{role:'bot',text:'👋 Hi! I\'m your PHLedger assistant.\nType help to see all commands.'}]);
  ...
  const [migD, setMigD]         = useState(null);
  const [migR, setMigR]         = useState(false);
  const [toast, setToast]       = useState({show:false,msg:'',ty:'success'});

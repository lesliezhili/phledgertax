...
export function handleChat(message) {
  ...
  // ── migrate: call /api/migrate (POST) and return summary ────────────
  if (msg === 'migrate') {
    return {
  ...
  // ── sync supabase: call /api/migrate?target=supabase (POST) ─────────
  if (msg === 'sync supabase' || msg === 'sync' || msg === 'upload' || msg === 'push supabase') {
    return {
      message: 'Syncing all transactions to Supabase...',
  ...
  // ─── SilverConnect commands ───────────────────────────────────────────
  if (msg === 'sc status' || msg === 'sc') {
    return { message: 'Loading SilverConnect data...', action: 'sc_load' };
  }
  if (msg === 'sc config' || msg === 'sc settings') {
    return { message: 'Opening SilverConnect config...', action: 'sc_config' };
  }
  if (msg === 'sc payouts' || msg === 'provider payouts') {
    return { message: 'Loading provider payout summary...', action: 'sc_payouts' };
  }
  if (msg.includes('sc p&l') || msg.includes('sc pl') || msg.includes('sc profit')) {
    return { message: 'Loading SilverConnect platform P&L...', action: 'sc_pl' };
  }

  // ─── KPI Management commands ────────────────────
  if (msg === 'kpi report' || msg === 'kpi scorecard' || msg === 'kpi') {
    return { message: 'Opening KPI management report...', action: 'kpi_report' };
  }
  if (msg === 'kpi targets' || msg === 'set targets') {
    return { message: 'Opening KPI targets editor...', action: 'kpi_targets' };
  }
  if (msg.includes('kpi summary') || msg.includes('kpi status')) {
    return { message: 'Loading KPI summary...', action: 'kpi_report' };
  }
